...
export default function gameEndingStatus(colour: Colour): EndingStatus {
	...
	for (let i = 1; i <= 8; i++) {
		for (let j = 1; j <= 8; j++) {
			...
			if (pieces.getColour(startCell) !== colour) continue;
			if (findAllMoves(startCell).length > 0) noValidMoves = false;
			if (!noValidMoves) break outer;
		}
	}

	if (noValidMoves) return isCheck(colour) ? 'checkmate' : 'stalemate';
	else return false;
}
